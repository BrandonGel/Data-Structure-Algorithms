...
int hashTable::insert(const std::string &key, void *pv)
{
	...
	// Find if there exists a key in the hashtable
	// Return 1 if found
	int h = hash(key);
	while (data[h].isOccupied == true)
	{
		if (data[h].key == key)
		{
			return 1;  
...
bool hashTable::contains(const std::string &key)
{
	...
	while (data[h].isOccupied == true) 
	{
		if (data[h].key == key)  // If yes 
			return true;
		h = (h + 1) % capacity; // Check the next index 
	}							//(the modulus allow the index to transverse back to the beginning of hashtable)
	return false;	// If no
...
{
	//Set the position of the key in the dictionary
	unsigned int h = 7;
	for (int i = 0; i < key.length(); i++) 
		h = h*31 + key[i];
	h %= capacity; //Added this to prevent h from going over the size of the dictionary
	return h;
}
